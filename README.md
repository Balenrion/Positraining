...
- **Cardio** → dis d'abord ton **objectif** (récupération active, raffermir, perdre de la cellulite, améliorer le cardio, fractionné/HIIT, ou libre) : l'app te propose un type de séance et une durée adaptés (modifiables), avec des repères concrets (inclinaison, vitesse, résistance, coups/min…) **qui s'ajustent à la machine réellement choisie** — changer de machine (tapis, vélo, rameur, elliptique, stepper) recalcule immédiatement le repère, plutôt que de garder celui de la machine par défaut de l'objectif. Une fois la séance faite, indique le temps réel, et si tu veux, ton ressenti et une note libre (réglages utilisés…) — c'est enregistré et visible dans l'Agenda.
...
- [x] Le ressenti (facile/ok/trop dur) de la dernière fois s'affiche directement en jaune sur les boutons Ressenti de la séance en cours (vert pour le ressenti de cette semaine), pour mieux voir d'un coup d'œil comment adapter les poids
...
- [x] Séance Hyrox : les 6 stations sont de vrais exercices comme les autres séances (grille de séries à cocher, fiche technique, historique/suggestion de charge d'une semaine à l'autre)
- [x] Préparation Hyrox : jusqu'à 3 séances par semaine, chacune obligatoire ou optionnelle, en jour dédié ou en séance du soir — configurable à la création du programme et modifiable à tout moment depuis Réglages, sans recréer de profil (remplace une ou plusieurs séances, jamais ajoutées en plus)
- [x] « Leg curl » renommé « Leg curl allongé » (le cue supposait déjà cette position) et « Leg curl assis » ajouté comme exercice à part entière — les deux ciblent différemment les ischios, chacun garde sa propre prescription et son propre historique (migration automatique pour les séances déjà en cours, historique de charges conservé)
- [x] « Russian twist » et « Crunch oblique au sol » ajoutés aux exercices abdos disponibles dans l'abdos express, le programme classique et le programme focus
- [x] « Crunch croisé » (bicycle), « Toucher chevilles » et « Planche avec touche épaule » ajoutés aux exercices abdos disponibles, mêmes emplacements
